...
OpenGLGPUTimer::OpenGLGPUTimer() : _numQueries( 0 ),  _queryFrame( 0 ), _time( 0 ), _activeQuery( false )
{
	reset();
}
...
IGPUTimer* OpenGLGPUTimer::factoryFunc()
{
	return new OpenGLGPUTimer();
}

void OpenGLGPUTimer::beginQuery( uint32 frameID )
{
...
bool OpenGLGPUTimer::updateResults()
{
...
IRenderDevice* OpenGLRenderDevice::factoryFunc()
{
	return new OpenGLRenderDevice();
}

void OpenGLRenderDevice::initStates()
{
...
uint32 OpenGLRenderDevice::registerVertexLayout( uint32 numAttribs, VertexLayoutAttrib *attribs )
{
...
uint32 OpenGLRenderDevice::createVertexBuffer( uint32 size, const void *data )
{
	RDIBuffer buf;
...
uint32 OpenGLRenderDevice::createIndexBuffer( uint32 size, const void *data )
{
	RDIBuffer buf;
...
void OpenGLRenderDevice::destroyBuffer( uint32 bufObj )
{
	if( bufObj == 0 ) return;
...
void OpenGLRenderDevice::updateBufferData( uint32 bufObj, uint32 offset, uint32 size, void *data )
{
...
uint32 OpenGLRenderDevice::calcTextureSize( TextureFormats::List format, int width, int height, int depth )
{
	switch( format )
	{
...
uint32 OpenGLRenderDevice::createTexture( TextureTypes::List type, int width, int height, int depth,
...
void OpenGLRenderDevice::uploadTextureData( uint32 texObj, int slice, int mipLevel, const void *pixels )
{
	...
	if( tex.genMips && (tex.type != TextureTypes::TexCube || slice == 5) )
	{
		...
		glEnable( getNativeTextureType( tex.type ) );  // Workaround for ATI driver bug
		glGenerateMipmapEXT( getNativeTextureType( tex.type ) );
		glDisable( getNativeTextureType( tex.type ) );
	}
...
void OpenGLRenderDevice::destroyTexture( uint32 texObj )
{
	if( texObj == 0 ) return;
...
void OpenGLRenderDevice::updateTextureData( uint32 texObj, int slice, int mipLevel, const void *pixels )
{
	uploadTextureData( texObj, slice, mipLevel, pixels );
}


bool OpenGLRenderDevice::getTextureData( uint32 texObj, int slice, int mipLevel, void *buffer )
{
	const RDITexture &tex = _textures.getRef( texObj );
...
uint32 OpenGLRenderDevice::createShaderProgram( const char *vertexShaderSrc, const char *fragmentShaderSrc )
{
	int infologLength = 0;
	int charsWritten = 0;
	char *infoLog = 0x0;
...
bool OpenGLRenderDevice::linkShaderProgram( uint32 programObj )
{
	int infologLength = 0;
	int charsWritten = 0;
	char *infoLog = 0x0;
...
uint32 OpenGLRenderDevice::createShader( const char *vertexShaderSrc, const char *fragmentShaderSrc )
{
...
void OpenGLRenderDevice::destroyShader( uint32 shaderId )
{
...
void OpenGLRenderDevice::bindShader( uint32 shaderId )
{
	if( shaderId != 0 )
	{
		RDIShader &shader = _shaders.getRef( shaderId );
...
int OpenGLRenderDevice::getShaderConstLoc( uint32 shaderId, const char *name )
{
	RDIShader &shader = _shaders.getRef( shaderId );
...
int OpenGLRenderDevice::getShaderSamplerLoc( uint32 shaderId, const char *name )
{
	RDIShader &shader = _shaders.getRef( shaderId );
	return glGetUniformLocation( shader.oglProgramObj, name );
}
...
void OpenGLRenderDevice::setShaderConst( int loc, RDIShaderConstType type, void *values, uint32 count )
{
	switch( type )
	{
	case CONST_FLOAT:
...
void OpenGLRenderDevice::setShaderSampler( int loc, uint32 texUnit )
{
...
uint32 OpenGLRenderDevice::createRenderBuffer( uint32 width, uint32 height, TextureFormats::List format,
...
void OpenGLRenderDevice::destroyRenderBuffer( uint32 rbObj )
{
...
uint32 OpenGLRenderDevice::getRenderBufferTex( uint32 rbObj, uint32 bufIndex )
{
	RDIRenderBuffer &rb = _rendBufs.getRef( rbObj );
...
void OpenGLRenderDevice::resolveRenderBuffer( uint32 rbObj )
{
	RDIRenderBuffer &rb = _rendBufs.getRef( rbObj );
...
void OpenGLRenderDevice::setRenderBuffer( uint32 rbObj )
{
...
bool OpenGLRenderDevice::getRenderBufferData( uint32 rbObj, int bufIndex, int *width, int *height,
...
uint32 OpenGLRenderDevice::createOcclusionQuery()
{
	uint32 queryObj;
...
void OpenGLRenderDevice::destroyQuery( uint32 queryObj )
{
...
void OpenGLRenderDevice::beginQuery( uint32 queryObj )
{
	glBeginQuery( GL_SAMPLES_PASSED, queryObj );
}


void OpenGLRenderDevice::endQuery( uint32 /*queryObj*/ )
{
	glEndQuery( GL_SAMPLES_PASSED );
}


uint32 OpenGLRenderDevice::getQueryResult( uint32 queryObj )
{
	uint32 samples = 0;
...
void OpenGLRenderDevice::checkGLError()
{
...
bool OpenGLRenderDevice::applyVertexLayout()
{
...
void OpenGLRenderDevice::applySamplerState( RDITexture &tex )
{
...
bool OpenGLRenderDevice::commitStates( uint32 filter )
{
...
void OpenGLRenderDevice::resetStates()
{
...
void OpenGLRenderDevice::clear( uint32 flags, float *colorRGBA, float depth )
{
	uint32 mask = 0;
...
void OpenGLRenderDevice::draw( RDIPrimType primType, uint32 firstVert, uint32 numVerts )
{
	if( commitStates() )
	{
		uint32 type = GL_TRIANGLES;
		switch(primType)
		{
		case PRIM_TRILIST:
			type = GL_TRIANGLES;
		...
		case PRIM_TRISTRIP:
			...
			break;
		default:
            Modules::log().writeError("[OpenGLRenderDevice::draw] - Unknown vertex layout");
			break;
		}
		glDrawArrays( type, firstVert, numVerts );
	}
...
void OpenGLRenderDevice::drawIndexed( RDIPrimType primType, uint32 firstIndex, uint32 numIndices,
                                uint32 firstVert, uint32 numVerts )
{
	if( commitStates() )
	{
		...
		case PRIM_TRISTRIP:
			...
			break;
		default:
            Modules::log().writeError("[OpenGLRenderDevice::drawIndexed] - Unknown vertex layout");
			break;
		}
		uint32 nativeIndexFormat = GL_UNSIGNED_SHORT;
		switch(_indexFormat)
		{
		case IDXFMT_16:
			nativeIndexFormat = GL_UNSIGNED_SHORT;
			break;
		case IDXFMT_32:
			nativeIndexFormat = GL_UNSIGNED_INT;
			break;
		default:
            Modules::log().writeError("[OpenGLRenderDevice::drawIndexed] - Unknown index format");
			break;
		}

		glDrawRangeElements( type, firstVert, firstVert + numVerts,
		                     numIndices, nativeIndexFormat, (char *)0 + firstIndex );
	}
