...
void main( void )
{
...
#ifdef _F02_NormalMapping
	mediump vec3 normalMapVec = texture2D( normalMap, newCoords.st ).rgb * 2.0 - 1.0;
	mediump vec3 normal = tsbMat * normalMapVec;
#else
	mediump vec3 normal = tsbNormal;
#endif
	...
	gl_FragColor.rgb = calcPhongSpotLight( newPos, normalize( normal ), albedo.rgb, matSpecParams.rgb,
